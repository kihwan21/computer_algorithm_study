...
def identity_functions(x):  # 값을 출력하기 위한 항등 함수(코드 흐름을 동일하게 하려고 있음) 설계는 미구현 선형 / 비선형
    return x
...
def forward(network, x):
    ...
    y = identity_functions(a3)  # 출력 함수 코드 흐림 때문에 넣은 함수(항등함수). 기능은 없음 출력층 설계는 미구현.  선형 / 비선형
